...
function Header(){
    ...
    $this->Cell(190,4,utf8_decode('Nómina de Alumnos/as - Por Género'),0,1,'C');
...
function FancyTable($header){
    ...
    $w=array(10,15,80,12.5,12.5,12.5,12.5,12.5,12.5,12.5,12.5); //determina el ancho de las columnas
    $w2=array(10,5,12,80,37.5,25,15); //determina el ancho de las columnas

       $this->Cell($w2[0],$w2[0],'','LTR',0,'L',1);
       $this->Cell($w2[6],$w2[0],'','LTR',0,'C',1);
       $this->Cell($w2[3],$w2[0],'','LTR',0,'C',1);
       
       $this->Cell($w2[4],$w2[0],'','LTBR',0,'C',1);
       $this->Cell($w2[5],$w2[0],'','LTBR',0,'C',1);
       $this->Cell($w2[4],$w2[0],'','LTBR',0,'C',1);

       $this->LN();
       
    for($i=0;$i<count($header);$i++)
        $this->Cell($w[$i],7,$header[$i],'LBR',0,'C',1);
    $this->Ln();
...
// Salto de línea.
    ...
    //cabecera
    //(numero, nie, nombre, edad, genero, col1,col2,col3,col4,col5,col6)
    $w=array(10,15,80,12.5,15,15,15,15,15,12.5); //determina el ancho de las columnas
    ...
    $fill=false; $i=1; $m = 0; $f = 0; $suma = 0; $cambiar = true;
	    ...
	    $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col1
	    $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col2
	    $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col3
	    $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col4
	    $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col5
	    $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col6
            $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col7
	    $pdf->Cell($w[3],5.8,'','LR',0,'C',$fill);  // col8
		...
		if($cambiar == true){
		  ...
		  $j = 1;
			...
			$pdf->Cell($w[2],5.8,'','LR',0,'C',$fill);  // NIE
            ...
            $pdf->Cell($w[0],5.8,$j,'LR',0,'C',$fill);        // núermo correlativo
            $pdf->Cell($w[1],5.8,trim($row['codigo_nie']),'LR',0,'C',$fill);  // NIE
            $pdf->Cell($w[2],5.8,utf8_decode(trim($row['apellido_alumno'])),'LR',0,'L',$fill); // Nombre + apellido_materno + apellido_paterno
