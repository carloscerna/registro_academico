...
/////////////////////////////////////////////////////////////////////////////////////////////
    switch($CodigoArea){
      case ($CodigoArea == "01" || $CodigoArea == "03"): 
        // tomar en cuenta el Técnico Modular.
        if($codigo_bachillerato == "15"){
            contar_promovidos_media_modular($generos, $notas, $contar_evaluar);
        }else{
    ...
    function Header()
    {
        ...
        {    
          ...
          // cuarta PARTE DEL RECTANGULO. asignatura
              ...
              $this->SetXY(102,5);
              $this->Cell(36,8,'BASICA',1,2,'C',true);
            $this->SetXY(138,5);
              $this->Cell(72,8,'TECNICA',1,2,'C',true);
          // quinta PARTE DEL RECTANGULO. educacion moral y civica
              $this->SetFont('Arial','',6); // I : Italica; U: Normal;
              $this->SetXY(210,5);
              $this->Cell(8,8,'COMP',1,2,'C',true);
          ...
          // cuarta PARTE DEL RECTANGULO. asignaturas nombres
              ...
              for($ix=0;$ix<=$total_asignaturas-1;$ix++){
                if($ix >= 0 && $ix <= 5){
                  $this->Rect(102+$espacio,13,6,42);
                  $this->RotatedTextMultiCellAspectos(103+$espacio,55,$nombre_asignatura[$ix],90);
                  $espacio = $espacio + 6;
                }else if($ix >=6 && $ix <= 15){
                  $this->SetFont('Arial','',6); // I : Italica; U: Normal;
                  $this->Rect(102+$espacio,13,8,42);
                  $this->RotatedTextMultiCellAspectos(102+$espacio,55,$nombre_asignatura[$ix],90);
                  $espacio = $espacio + 8;
                }
                else{
                  $this->SetFont('Arial','',8); // I : Italica; U: Normal;
...
// cuarta PARTE DEL RECTANGULO. asignatura
    ...
    $pdf->SetXY(102,45);
    $pdf->Cell(40,8,'BASICA',1,2,'C',true);
    $pdf->SetXY(138,45);
    $pdf->Cell(72,8,'TECNICA',1,2,'C',true);
// quinta PARTE DEL RECTANGULO. educacion moral y civica
    $pdf->SetFont('Arial','',6); // I : Italica; U: Normal;
    $pdf->SetXY(210,45);
    $pdf->Cell(8,8,'COMP',1,2,'C',true);
// sexta PARTE DEL RECTANGULO. educacion moral y civica
...
// cuarta PARTE DEL RECTANGULO. asignaturas nombres
    ...
    for($ix=0;$ix<=$total_asignaturas-1;$ix++){
      if($ix >= 0 && $ix <= 5){
        $pdf->Rect(102+$espacio,53,6,42);
        $pdf->RotatedTextMultiCellAspectos(103+$espacio,95,$nombre_asignatura[$ix],90);
        $espacio = $espacio + 6;
      }else if($ix >=6 && $ix <= 15){
        $pdf->SetFont('Arial','',6); // I : Italica; U: Normal;
...
//Datos para nombres, asignaturas.
    ...
    $fill = true;$i=1;  $suma = 0; $numero = 1; $nota_concepto = 0; $conteo_alumnos = 0; 
     ...
     $w=array(6,10,8);  // determinar el ancho de
     $result = $db_link -> query($query);
		while($row = $result -> fetch(PDO::FETCH_BOTH))
          {
            ...
            switch($i)
            {
              ...
              case ($i>=2 && $i<=15):
                  ...
                  // Modular.
                  if($i == 7){$total_puntos_07_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 8){$total_puntos_08_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 9){$total_puntos_09_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 10){$total_puntos_10_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 11){$total_puntos_11_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 12){$total_puntos_12_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 13){$total_puntos_13_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 14){$total_puntos_14_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                  if($i == 15){$total_puntos_15_array[] = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);}
                // comparar si es básica o modulos.
                if($i>=2 && $i<=6){
                    $nota_final_ = verificar_nota_media($nota_final, $nota_r_1, $nota_r_2);
                    if($nota_final_ < 6 ){
                    $pdf->SetTextColor(255,0,0);
                    ...
                    $nota_final_ = verificar_nota_media_contable($row['nota_final'],$row['recuperacion']);
                    if($nota_final_ < 3 ){
                    $pdf->SetTextColor(255,0,0);
                        $pdf->Cell($w[2],$h[0],$nota_final_,1,0,'C',$fill); 
                    $pdf->SetTextColor(0,0,0);
                    }else{
                    $pdf->Cell($w[2],$h[0],$nota_final_,1,0,'C',$fill); 
                    }
              ...
              case $i == 16:  // muci
                ...
                $pdf->Cell(8,$h[0],$concepto_asignatura,1,0,'C',$fill); break;
              case ($i>=17 && $i<=19):  // competencias ciudadanas.
              	$nota_concepto = verificar_nota_media($nota_final, $nota_r_1, $nota_r_2);
                $concepto_asignatura = cambiar_concepto($nota_concepto);
              ...
              if($numero == 23 && $i == $total_asignaturas){
                ...
                //Crear una línea. Fecha.
                    ...
                    $pdf->Line(255,70,350,70);
                //Crear una línea. F. Docente.
                    $pdf->RotatedText(258,92,'F:',0);
                    $pdf->Line(255,95,350,95);
                ...
                //Crear una línea. F. Director.
                    $pdf->RotatedText(258,122,'F:',0);
                    $pdf->Line(255,125,350,125);
		   ...
		   // Para el fondo de la fila.
		      $fill=!$fill;		    
                    $pdf->SetX(10);
                      ...
                      for($j=0;$j<=5;$j++){$pdf->Cell($w[0],$h[0],'',1,0,'C');}
                      // segundo bloque del ancho.
                      for($j=0;$j<=9;$j++){$pdf->Cell($w[2],$h[0],'',1,0,'C');}
                      // tercer bloque
                      for($j=0;$j<=3;$j++){$pdf->Cell($w[1],$h[0],'',1,0,'C');}
                      $pdf->Ln();
             ...
             // Salto de página.
                ...
                $pdf->Addpage();
        				$pdf->SetY(75);
                // cuarta PARTE DEL RECTANGULO. cuadro de la firma.
                    $pdf->Rect(280,150,50,40);
                    $pdf->RotatedText(300,187,'SELLO',0);
                //Crear una línea. Lugar. Line(x hacia la izq.,y - mueven hacia abajo,x1 hacia a la izq.,y1 mueven hacia abajo)
                    $pdf->RotatedText(262,35,'Lugar:',0);
                    $pdf->RotatedTextMultiCellDireccion(295-((strlen($_SESSION['direccion']))/2),37,$_SESSION['direccion'],0);
                    $pdf->Line(262,45,350,45);
                    $pdf->SetY(55);
                //Crear una línea. Fecha.
                    $pdf->RotatedText(262,60,'Fecha:',0);
                    $pdf->RotatedText(265,67,trim($_SESSION['dia_entrega'])." de ".$mes." de ".convertirtexto(strtolower(num2letras($año))),0);
                    $pdf->Line(262,70,350,70);
                //Crear una línea. F. Docente.
                    $pdf->RotatedText(262,92,'F:',0);
                    $pdf->Line(262,95,350,95);
                    $pdf->RotatedText(300-((strlen(trim($nombre_encargado)))/2),99,cambiar_de_del($nombre_encargado),0);
                    $pdf->RotatedText(300-((strlen('Docente'))/2),105,'Docente',0);
                //Crear una línea. F. Director.
                    $pdf->RotatedText(262,122,'F:',0);
                    $pdf->Line(262,125,350,125);
                    $pdf->RotatedText(295-((strlen(trim($nombre_director)))/2),132,cambiar_de_del($nombre_director),0);
                    $pdf->RotatedText(300-((strlen('Director'))/2),137,'Director',0);
		}
		...
		for($i=0;$i<=$linea_faltante;$i++)
           ...
           // Ultimas lineas....
              $pdf->SetX(10);
				...
				$pdf->Cell($w[2],$h[0],array_sum($total_puntos_07_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_08_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_09_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_10_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_11_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_12_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_13_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_14_array),1,0,'C');
                $pdf->Cell($w[2],$h[0],array_sum($total_puntos_15_array),1,0,'C');
                 // 
                for($j=0;$j<=4;$j++){
                    if($j == 0){
                        $pdf->Cell(8,$h[0],'',1,0,'C');
                    }else{
                        $pdf->Cell(10,$h[0],'',1,0,'C');
                    }
                }
                //
                $pdf->Ln();
                ...
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_07_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_08_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_09_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_10_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_11_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_12_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_13_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_14_array)/$conteo_alumnos,0),1,0,'C');
                $pdf->Cell($w[2],$h[0],number_format(array_sum($total_puntos_15_array)/$conteo_alumnos,0),1,0,'C');
                    $pdf->SetTextColor(0);
                    $pdf->SetFont('');
