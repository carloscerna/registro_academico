...
function Footer()
{
    ...
    //N�mero de p�gina
    $this->SetY(-10);
    $this->Cell(0,6,utf8_decode('N� ').$this->PageNo().'/{nb}',0,0,'C');
...
// cuarta PARTE DEL RECTANGULO. asignatura
    ...
    $this->Rect(91,35,70,20);
    $this->Rect(91,35,70,5);
    $this->SetXY(120,35);
...
// OCTAVA PARTE DEL RECTANGULO. Total de Puntos.
    $this->Rect(161,35,8,20);
    $this->SetFont('Arial','',9); // I : Italica; U: Normal;
    $this->RotatedText(166,54,'T.Repo',90);

// NOVENA PARTE DEL RECTANGULO. DERECHO A RECUPERACI�N. (D. A  R.)
...
// Definimos el tipo de fuente, estilo y tama�o.
        $w2=array(5); //determina el ancho de las columnas
            $pdf->SetFont('Arial','',10); // I : Italica; U: Normal;
             ...
             $pdf->Cell(50,$w2[0],utf8_decode('Secci�n: ').$print_seccion,0,0,'L');
             $pdf->Cell(30,$w2[0],utf8_decode('A�o Lectivo: ').$print_ann_lectivo,0,0,'L');
            $pdf->ln();
     ...
     while($row = $result -> fetch(PDO::FETCH_BOTH))
		  ...
		  //acumular el valor - matematica.
            ...
            // validar $i = 11
                ...
                {
                    if($total_reprobadas > 2){$derecho_recuperacion = 'No';}else{$derecho_recuperacion = "S�";}
...
// SEPTIMA PARTE DEL RECTANGULO.
    ...
    {    
	    ...
	    $pdf->Cell($w[2],$w2[0],$tapaf,0,1,'C',$fill);
	    /*
	    ...
	    $pdf->Cell($w[2],$w2[0],$treaf,0,1,'C',$fill);
/*
	    ...
	    $pdf->Cell($w[2],$w2[0],$tapaf+$treaf,0,1,'C',$fill);
/*
	    ...
	    $pdf->Cell($w[4],$w2[0],count($aprobada_ingles) + count($reprobada_ingles),0,1,'C',$fill);
	    /*
