...
// archivos que se incluyen.
    ...
    include($path_root."/registro_web/includes/DeNumero_a_Letras.php");
...
function Footer()
{
    ...
    //Firma Director.
    if($firma == 'yes'){
      $img = $_SERVER['DOCUMENT_ROOT'].'/registro_web/img/'.$_SESSION['imagen_firma'];;
      $this->Image($img,130,130,70,15);
    }
    if($sello == 'yes'){
      $img_sello = $_SERVER['DOCUMENT_ROOT'].'/registro_web/img/'.$_SESSION['imagen_sello'];;
  ...
  if ($print_codigo_bachillerato >= '01' and $print_codigo_bachillerato <= '05'){
    ...
    $fill = false; $i=1;  $suma = 0; $aprobado_reprobado = array();
		...
		// imprimir la foto en la boleta
                    if ($chkfoto == 'yes'){
                        if(!file_exists($_SERVER['DOCUMENT_ROOT'].'/registro_web/img/png'.'/'.$foto)){$fotos = 'foto_no_disponible.png';}else{$fotos = $foto;}
                            $img = $_SERVER['DOCUMENT_ROOT'].'/registro_web/img/png'.'/'.$fotos;
