		...
		while($objPHPExcel->getActiveSheet()->getCell("A".$fila)->getValue() != "")
		  {
			 // Extraer datos de la Hoja de Cálculo.
                ...
                */
                ...
                $distancia = $objPHPExcel->getActiveSheet()->getCell("S".$fila)->getValue();        // DISTANCIA AL CENTRO EDUCATIVO
                $sintoniza = $objPHPExcel->getActiveSheet()->getCell("T".$fila)->getValue();        // PUEDE SINTONIZAR EN SU  CASA CANAL 10
                if($sintoniza == "No"){$bollean_sintoniza = 'false';}else{$bollean_sintoniza = 'true';}

                $franja_educativa = $objPHPExcel->getActiveSheet()->getCell("U".$fila)->getValue();        // SINTONIZA LA FRANJA EDUCATIVA 
                if($franja_educativa == "No"){$bollean_franja_educativa = 'false';}else{$bollean_franja_educativa = 'true';}

                $cantidad_personas = $objPHPExcel->getActiveSheet()->getCell("V".$fila)->getValue();        // CANTIDAD DE PERSONAS QUE VIVEN CON EL ESTUDIANTE
                $viven_personas_menores = $objPHPExcel->getActiveSheet()->getCell("W".$fila)->getValue();        // VIVEN PERSONAS MENORES DE 18 AÑOS
