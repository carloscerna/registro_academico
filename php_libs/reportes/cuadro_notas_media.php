...
//    $this->Image('../registro_academico/img/'.$_SESSION['logo_uno'],10,8,33);
    ...
    $this->Cell(250,4,utf8_decode('Cuadro de Registro de Notas Por Período'),0,1,'C');
    //Salto de línea
   // $this->Ln(20);
}
...
function FancyTable($header)
{
    ...
    $this->SetFillColor(203,204,204);
    $this->SetTextColor(0,0,0);
    $this->SetDrawColor(0,0,0);
    $this->SetLineWidth(.3);
    $this->SetFont('','B');
    //Cabecera
    $w=array(5,12,70,40,40,40,40,10,10,10); //determina el ancho de las columnas
    $w1=array(10,15,10); //determina el ancho de las columnas
    ...
    for($j=0;$j<=3;$j++)
        ...
        $this->Cell($w1[2],7,utf8_decode('4 Pe.'),'LBR',0,'C',1);   // lineas para el total de puntos
        $this->Cell($w1[2],7,'Repo','LBR',0,'C',1);    // lineas para el total de puntos
        $this->Cell($w1[2],7,'Final','LBR',0,'C',1);    // lineas para el total de puntos
    $this->Ln();
...
// Creando el Informe.
    ...
    $pdf->SetMargins(1, 15, 5);
    #Establecemos el margen inferior: 
    $pdf->SetAutoPageBreak(true,10);
    
    $data = array();
//Títulos de las columnas
    $header=array('Nº','N I E','Nombre de Alumnos/as','Primer Período','Segundo Período','Tercer Período','Cuarto Período','Prom.','Nota','Prom.');
    $pdf->AliasNbPages();
...
//  mostrar los valores de la consulta
    $w=array(5,12,70,10,10,10); //determina el ancho de las columnas
    ...
    $fill = false; $i=1;
     while($row = $result -> fetch(PDO::FETCH_BOTH))
            ...
            $i=$i+1;
                if($i==26 || $i == 51){
		  $pdf->Cell(array_sum($w)+(10*11)+(15*2)+20,0,'','T');
		  $pdf->AddPage();
		      $pdf->Cell(140,8,$print_bachillerato,0,0,'L');
		      $pdf->Cell(40,8,$print_grado,0,0,'L');
		      $pdf->Cell(20,8,$print_seccion,0,0,'L');
