...
function Header()
{
 ...
 //Colores, ancho de línea y fuente en negrita
		...
		$this->SetFillColor(255);
		$this->Rect(5,40,7,15); // Número.
		$this->Rect(5,40,87,15); // Orden Alfabetico
		$this->Rect(5,40,102,15); // T1
		$this->Rect(5,40,117,15); // T2
        ...
        while($row = $result -> fetch(PDO::FETCH_BOTH))
            {
			   // >Impresión de los promedios para las asignaturas.
                if($codigo_bachillerato == '05' || $codigo_bachillerato == '04'){
						...
						if(trim($row['nota_p_p_3']) <> 0){$pdf->Cell($w[4],$h[0],trim($row['nota_p_p_3']),0,0,'C',$fill);}else{$pdf->Cell($w[4],$h[0],'',0,0,'C',$fill);}
						// total de puntos
						$pdf->SetFont('Arial','B',11);
						$pdf->Cell($w[5],$h[0],trim($row['total_puntos_basica']),0,0,'C',$fill);
         ...
         $print_nombre = $nombre_asignatura . "-" . $nombre_grado . "-" . $nombre_seccion . '.pdf';
