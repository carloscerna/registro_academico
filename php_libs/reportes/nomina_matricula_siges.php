...
function FancyTable()
{
    //Colores, ancho de línea y fuente en negrita
		...
		$contenido_encabezado = array('Nº','Id','Número de Identificación Estudiantil (NIE)','Nombre Completo del Alumno/a (Por Apellidos)','Género M/F','Fecha de nacimiento dd/mm/aaaa','Estado Civil*','Nacionalidad','Partida Nacimiento (Si/No)'
									  ,'Medio de Transporte al C.E.','Distancia de residencia al C.E. (Km.)','Datos de P.N.','Código Depar./Municipio*','Teléfono/Residencia/Celular');
...
function FancyTable2()
{
    //Colores, ancho de línea y fuente en negrita
		...
		//Cabecera tamaño de ancho y alto y array para los encabezados.
		$contenido_encabezado = array('Nº','Zona Resid. Urb./Rur.','Rep. Grad.Sí/No','Estu. Parv. Sí/No','Tipo Discap.','Activ. Econó.','Est. Fam.','Tiene Hijos','Sí, Cant. Hijos','Año est. Grado Ant.','Nombre del Padre/Madre/Encargado','Lugar de Trabajo','Profesión u Oficio','F. Nac. / Nº DUI','Teléfono','Dirección','Responsable Padre/Madre/Otro','FIRMA');
		//un arreglo con su medida  a lo ancho
...
//Títulos de las columnas
    ...
    $query_encargado = "SELECT eg.id_encargado_grado, btrim(p.nombres || CAST(' ' AS VARCHAR) || p.apellidos) as nombre_docente, eg.codigo_encargado, bach.nombre, gann.nombre, sec.nombre, ann.nombre
     ...
     $query = "SELECT a.id_alumno, a.codigo_nie, btrim(a.apellido_paterno || CAST(' ' AS VARCHAR) || a.apellido_materno || CAST(', ' AS VARCHAR) || a.nombre_completo) as apellido_alumno, 
		...
		a.codigo_discapacidad, a.codigo_municipio, a.codigo_departamento,
		am.codigo_bach_o_ciclo, am.pn, bach.nombre as nombre_bachillerato, ae.codigo_familiar,
        ...
        sec.nombre as nombre_seccion, ae.codigo_alumno, ae.nombres, ae.encargado, ae.dui, ae.lugar_trabajo, ae.profesion_oficio, ae.direccion as direccion_encargado, ae.telefono, am.certificado, am.tarjeta_vacunacion, a.direccion_alumno, a.telefono_alumno, ae.fecha_nacimiento as encargado_fecha_nacimiento,
    ...
    cat_familiar.descripcion as responsable,
    ...
    INNER JOIN catalogo_familiar cat_familiar ON cat_familiar.codigo = ae.codigo_familiar
...
// Crear el encabezando a la izquierda y derecha.
		...
		$pdf->RotatedText(10,9,'DATOS DE MATRICULA',0);
...
//  cuenta el total de alumnos para colocar en la estadistica.
    ...
    {   
       // Imprimir valores si $contar_p_m >= 6 ó $contar_p_f >= 6. // y para mostrar los aprobados.
				...
				$datos_pn = 'Nº P.N. --> '.trim($row['pn_numero']) . ' - ' . 'Folio --> '.trim($row['pn_folio']) . ' - ' . 'Tomo --> '.trim($row['pn_tomo']) . ' - ' . 'Libro --> '.trim($row['pn_libro']);
        $codigo_departamento_municipio = trim($row['codigo_departamento']) . trim($row['codigo_municipio']);
				$telefono_alumno_celular = trim($row['telefono_alumno']) . '/' .trim($row['telefono_celular']);
				 ...
				 //Cabecera tamaño de ancho y alto y array para los encabezados.
				 $contenido_tabla = array($numero,$id_alumno,$codigo_nie,$apellido_alumno,$nombre_genero,$fecha_nacimiento,$nombre_estado_civil,$nombre_nacionalidad,$partida_nacimiento,$nombre_transporte,$distancia,$datos_pn,$codigo_departamento_municipio,$telefono_alumno_celular);
				 //un arreglo con su medida  a lo ancho
...
//  cuenta el total de alumnos para colocar en la estadistica.
    ...
    {   
        // Asignar valores a variables.
				...
				$dui = trim($row['dui']);
				$encargado_fecha_nacimiento = cambiaf_a_normal($row['encargado_fecha_nacimiento']);
				$e_dui = ' * '.$encargado_fecha_nacimiento . ' * ' . $dui;
				$telefono = trim($row['telefono']);
				//$direccion = trim($row['direccion_encargado']);
        $nombre_departamento = trim($row['nombre_departamento']);
        $nombre_municipio = trim($row['nombre_municipio']);
        $n_depa_muni = $nombre_departamento . ' - '. $nombre_municipio . '-> ';
        $direccion =  ucfirst(strtolower(trim($row['direccion_alumno'])));
        $responsable = trim($row['responsable']);
				...
				if($estudio_parvularia == '1'){$estudio_parvularia = "Sí";}else{$estudio_parvularia = "No";}
				 ...
				 $contenido_tabla = array($numero,$nombre_zona,$repitente,$estudio_parvularia,$codigo_discapacidad,$codigo_actividad_economica,$codigo_estado_familiar,$tiene_hijos,$cantidad_hijos,$ann_estudio_anterior,$nombre_encargado,$lugar_trabajo,$profesion_u_oficio,$e_dui,$telefono,$n_depa_muni.$direccion,$responsable,'');
				 //un arreglo con su medida  a lo ancho
	 ...
	 //			$yy = $pdf->GetY();
				 ...
				 $pdf->Row(array(utf8_decode($contenido_tabla[0]),utf8_decode($contenido_tabla[1]),utf8_decode($contenido_tabla[2]),utf8_decode($contenido_tabla[3]),utf8_decode($contenido_tabla[4]),utf8_decode($contenido_tabla[5])
								 ...
								 ,utf8_decode($contenido_tabla[13]),utf8_decode($contenido_tabla[14]),utf8_decode($contenido_tabla[15]),utf8_decode($contenido_tabla[16])));			
				 // CORTE PARA COLOCAR DE NUEVO EL ENCABEZADO AL PRINCIPIO DE LA SIGUIENTE PÁGINA.
					 if($i == 11 or $i == 22 or $i == 33 or $i == 44)
					 {
						 $pdf->AddPage();
