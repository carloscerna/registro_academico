...
// variables y consulta a la tabla.
    ...
    $db_link = $dblink;
// Esperar x tiempo en ejecucion de script
    set_time_limit(0);
...
function Header()
{
    ...
    $this->Cell(135,7,utf8_decode($_SESSION['institucion']),0,1,'C');
...
//************************************************************************************************************************
// crear bucle para ver todo el grado completo y pasar los valores a las variables correspondientes.
    $codigo_all = $_REQUEST["todos"];
    consultas(8,0,$codigo_all,'','','',$db_link,'');
 // crear una matriz para ambos.
    $codigo_matricula_a = array(); $codigo_alumno_a = array();
 
 while($row = $result -> fetch(PDO::FETCH_BOTH))
    {
        // variables y consulta a la tabla.
        $codigo_matricula_a[] = $row['cod_matricula'];
        $codigo_alumno_a[] = $row['cod_alumno'];
    }
...
// generar la ficha del alumno en base al resultado de la matriz. con un for.
for($b=0;$b<count($codigo_matricula_a);$b++)
{    
...
// crear lineas y rect�ngulos.
    //Crear una l�nea de la primera firma a 55 cm.
    $pdf->Line(5,55,140,55);
    //Crear una l�nea de la primera firma a 85 cm.
    $pdf->Line(5,95,140,95);
    //Crear una l�nea de la primera firma a 140 cm.
    $pdf->Line(5,141,140,141);
    //Crear una l�nea de la primera firma a 175 cm.
...
// Definimos el tipo de fuente, estilo y tama�o.
    $pdf->SetFont('Comic','',8); // I : Italica; U: Normal;
// para el rect�ngulo del label bajar uno, uno menos del margen izq. y de alto 5, ancho depende del campo.
    ...
    $pdf->RotatedText(60,30,'Tel�fono: Casa',0);
    $pdf->Rect(59,31,20,5);
    
    $pdf->RotatedText(90,30,'C�lular',0);
    $pdf->Rect(89,31,20,5);
    
    $pdf->RotatedText(8,40,'Direcci�n',0);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $pdf->RotatedText(75,65,'G�nero',0);
    $pdf->Rect(74,66,20,5);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $pdf->RotatedText(54,85,'N�mero',0);
    $pdf->Rect(53,86,10,5);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
 $query_a = "SELECT  a.id_alumno, a.codigo_nie, btrim(a.apellido_paterno || CAST(' ' AS VARCHAR) || a.apellido_materno || CAST(', ' AS VARCHAR) || a.nombre_completo) as apellido_alumno,
        ...
        WHERE id_alumno = '$codigo_alumno_a[$b]' ORDER BY apellido_alumno";
        // ejecutar la consulta.
        $result = $db_link -> query($query_a);
    //**consultas_alumno(3,0,$codigo_all,$codigo_alumno_a[$b],$codigo_matricula_a[$b],$db_link,''); 
         while($row = $result -> fetch(PDO::FETCH_BOTH))
            {
		$pdf->RotatedText(122,18,$row['id_alumno'],0);
		...
		$pdf->RotatedText(60,35,trim($row['telefono_alumno']),0);
                $pdf->RotatedTextMulticell(8,42,utf8_decode(ucwords(strtolower(trim($row['direccion_alumno'])))),0);
		...
		$pdf->RotatedText(8,70,cambiar_de_del($row['nombre_departamento']),0);
		$pdf->RotatedText(41,70,cambiar_de_del($row['nombre_municipio']),0);
		$pdf->RotatedText(75,70,$genero,0);
		$pdf->RotatedText(98,70,cambiar_de_del($row['estado_civil']),0);
		
		$pdf->RotatedText(8,80,cambiar_de_del($row['nacionalidad']),0);
		$pdf->RotatedText(41,80,cambiar_de_del(trim($row['transporte'])),0);
		...
		$codigo_zona = cambiar_de_del($row['codigo_zona_residencia']);
		$estudio_parvularia = cambiar_de_del($row['estudio_parvularia']);
		$tiene_hijos = cambiar_de_del($row['tiene_hijos']);
		$cantidad_hijos = cambiar_de_del($row['cantidad_hijos']);
...
// colocar etiquetas y cuadros que sean necesarios.
    $pdf->SetTextColor(255,0,0);
    $pdf->RotatedText(30,100,'INFORMACI�N DEL PADRE, MADRE O ENCARGADO',0);
// colocar etiquetas y cuadros que sean necesarios.
...
// Definimos el tipo de fuente, estilo y tama�o.
	...
	$pdf->RotatedText(71,$eje_y_etiqueta_2,'Profesi�n u Oficio',0);
	$pdf->Rect(70,$eje_y_rectangulo_2,60,5);

	$pdf->RotatedText(8,$eje_y_etiqueta_3,'Direcci�n',0);
	$pdf->Rect(7,$eje_y_rectangulo_3,100,10);

	$pdf->RotatedText(111,$eje_y_etiqueta_3,'Tel�fono',0);
...
//  construir consulta.
		...
		WHERE id_alumno = $codigo_alumno_a[$b] ORDER BY ae.id_alumno_encargado";
    // ejecutar la consulta.
        $result = $db_link -> query($query);

    //consultas_alumno(4,0,$codigo_all,$codigo_alumno_a[$b],$codigo_matricula_a[$b],$db_link,'','');
    $eje_y_campo_1 = 113;
    $eje_y_campo_2 = 123;
    $eje_y_campo_3 = 130;   // para la direcci�n.
    $eje_y_campo_4 = 132;   // para el n�mero de telefono.
		...
		$pdf->RotatedTextMulticell(8,$eje_y_campo_3,utf8_decode(trim($row['direccion'])),0);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////AGREGAR LA SEGUNDA P�GINA//////////////////////////////////////////////////////////////////////////////////
//T�tulos de las columnas
    $header=array('Entidad','Serv.Educativo','Secci�n','A�o Lectivo','Estatus');
    $pdf->AliasNbPages();
    $pdf->AddPage();
...
// colocar etiquetas y cuadros que sean necesarios.
    ...
    $pdf->SetFont('Comic','',8); // I : Italica; U: Normal;    
    $pdf->RotatedText(8,30,'Actividad Econ�mica',0);
    $pdf->Rect(50,27,10,5);
...
WHERE am.codigo_alumno = $codigo_alumno_a[$b] ORDER BY nombre_ann_lectivo";
// ejecutar la consulta.
$result = $db_link -> query($query);

    //consultas_alumno(5,0,$codigo_all,$codigo_alumno_a[$b],$codigo_matricula_a[$b],$db_link,'');
        ...
        $pdf->Ln();
           while($row = $result -> fetch(PDO::FETCH_BOTH))
            {
		$linea_estudio++;
                $pdf->Cell($w[0],6,ucwords(strtolower(trim($row['nombre_escuela']))),'LR',0,'L',$fill);	// Entidad.
                $pdf->Cell($w[1],6,utf8_decode(trim($row['nombre_grado'])),'LR',0,'C',$fill);	// Servicio educativo
                $pdf->Cell($w[2],6,trim($row['nombre_seccion']),'LR',0,'C',$fill);	// Secci�n
                $pdf->Cell($w[3],6,trim($row['nombre_ann_lectivo']),'LR',0,'C',$fill);	// A�o Lectivo.
	    ...
	    for($li;$li<=$linea_faltante;$li++)
	    {
                ...
                $pdf->Cell($w[2],6,'','LR',0,'C',$fill);	// Secci�n
                $pdf->Cell($w[3],6,'','LR',0,'C',$fill);	// A�o Lectivo.
                $pdf->Cell($w[4],6,'','LR',1,'C',$fill);	// Estatus
                $fill=!$fill;
    }
// Cerrando L�nea Final.
