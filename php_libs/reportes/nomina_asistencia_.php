...
function FancyTable($header)
{
    ...
    $this->SetFont('','B');
    //Cabecera
    $w=array(10,80,170); //determina el ancho de las columnas
    ...
    //      $this->Ln();
        ...
        $this->Cell($w[1],7,'(Orden Alfabético por Apellido)','LBR',0,'C',1);
...
// Definimos el tipo de fuente, estilo y tamaño.
    ...
    $w=array(10,80,5.66); //determina el ancho de las columnas
    
    $fill=false; $i=1; $m = 0; $f = 0; $suma = 0;
        while($row = $result -> fetch(PDO::FETCH_BOTH))
            {
            $pdf->Cell($w[0],6,$i,'LR',0,'C',$fill);        // núermo correlativo
            $pdf->Cell($w[1],6,utf8_decode(trim($row['apellido_alumno'])),'LR',0,'L',$fill); // Nombre + apellido_materno + apellido_paterno
            for($j=0;$j<=29;$j++)
