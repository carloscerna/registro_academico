...
function Header()
{
	...
	$this->RoundedRect(250, 5, 20, 24, 3.5, '1234','');		// RECTANGULO de la foto.
...
// MATRIZ NOMBRE DE LOS CAMPOS. unset( $animales[0] ); BORRAR MATRIZ
	...
	if($crear_archivos == "si"){
		...
		if(!file_exists($DestinoArchivo . $nuevo_grado . ' ' . trim($nombreSeccion)))
		{
			// Para Nóminas. Escolanadamente.
				mkdir ($DestinoArchivo . $nuevo_grado . ' ' . trim($nombreSeccion));
				chmod($DestinoArchivo . $nuevo_grado . ' ' . trim($nombreSeccion),07777);
		}
			$NuevoDestinoArchivo = $DestinoArchivo . $nuevo_grado . ' ' . trim($nombreSeccion) . "/";
