...
// variables. del post.
    //$nombre_archivo = "CE10428-ER.xlsx";
		...
		while($objPHPExcel->getActiveSheet()->getCell("A".$fila)->getValue() != "")
		  {
			 // Extraer datos de la Hoja de Cálculo.
                ...
                */
                ...
                // ZONA RESIDENCIA.
                $zona_residencia = $objPHPExcel->getActiveSheet()->getCell("H".$fila)->getValue();        // ZONA RESIDENCIA
                if($zona_residencia == "Urbana"){$codigo_zona = "01";}else{$codigo_zona = "02";}

                $numero_dormitorio = $objPHPExcel->getActiveSheet()->getCell("I".$fila)->getValue();        // CUANTOS DORMITORIOS TIENE SU CASA
                $hogar_cuenta_con = $objPHPExcel->getActiveSheet()->getCell("J".$fila)->getValue();        // EN SU HOGAR, CUENTA CON LO SIGUIENTE

                $servicio_energia = $objPHPExcel->getActiveSheet()->getCell("K".$fila)->getValue();        // CUENTA CON SERVICIO DE ENERGIA ELECTRICA EN SU CASA
                if($servicio_energia == "No"){$bollean_servicio_energia = 'false';}else{$bollean_servicio_energia = 'true';}

                // ABASTECIMIENTO DE AGUA.
                $abastecimiento_agua = $objPHPExcel->getActiveSheet()->getCell("L".$fila)->getValue();        // CUAL ES LA FUENTE PRINCIPAL DE ABASTECIMIENTO DE AGUA DE SU CASA
                ...
                */
                ...
                $catalogo_abastecimiento_agua = array("Acarrero (río, lago, nacimiento de agua)","Agua Lluvias","Otros","Pila, chorro público o cantarea","Pipa","Pozo","Servicio de agua por cañeria interna a la casa");
                ...
                $material_piso = $objPHPExcel->getActiveSheet()->getCell("M".$fila)->getValue();        // CUAL ES EL MATERIAL PRINCIPAL DEL PISO DE SU CASA
                /*
                    1	"01"	"Cemento                  "
                ...
                */
                ...
                $servicio_sanitario = $objPHPExcel->getActiveSheet()->getCell("N".$fila)->getValue();        // QUE TIPO DE SERVICIO SANITARIO TIENE SU CASA
                ...
                */
                ...
                $conexion_internet = $objPHPExcel->getActiveSheet()->getCell("O".$fila)->getValue();        // TIENE ALGUN TIPO DE CONEXION A INTERNET RESIDENCIAL
                if(trim($conexion_internet) == 'No'){$bollean_conexion_internet = 'f';}else{$bollean_conexion_internet = 't';}

                $distancia = $objPHPExcel->getActiveSheet()->getCell("P".$fila)->getValue();        // DISTANCIA AL CENTRO EDUCATIVO
                $sintoniza = $objPHPExcel->getActiveSheet()->getCell("Q".$fila)->getValue();        // PUEDE SINTONIZAR EN SU  CASA CANAL 10
                if($sintoniza == "No"){$bollean_sintoniza = 'false';}else{$bollean_sintoniza = 'true';}

                $franja_educativa = $objPHPExcel->getActiveSheet()->getCell("R".$fila)->getValue();        // SINTONIZA LA FRANJA EDUCATIVA 
                if($franja_educativa == "No"){$bollean_franja_educativa = 'false';}else{$bollean_franja_educativa = 'true';}

                $cantidad_personas = $objPHPExcel->getActiveSheet()->getCell("S".$fila)->getValue();        // CANTIDAD DE PERSONAS QUE VIVEN CON EL ESTUDIANTE
                $viven_personas_menores = $objPHPExcel->getActiveSheet()->getCell("T".$fila)->getValue();        // VIVEN PERSONAS MENORES DE 18 AÑOS
                if($viven_personas_menores == "No"){$bollean_viven_personas_menores = 'false';}else{$bollean_viven_personas_menores = 'true';}

                // datos del encargado.
                $fecha_nacimiento = $objPHPExcel->getActiveSheet()->getCell("U".$fila)->getFormattedValue();        // FECHA DE NACIMIENTO
                //$excelTimestamp = 43106; //valor recogido de la celda del archivo excel
                //$fecha_nacimiento = ($fecha_nacimiento_);
                


                $direccion = $objPHPExcel->getActiveSheet()->getCell("V".$fila)->getValue();        // DIRECCION
                $numero_contacto = $objPHPExcel->getActiveSheet()->getCell("W".$fila)->getValue();        // NUMERO DE CONTACTO
                ...
                // VERIFICAR SI EXISTE EN ALUMNO_HOGAR
                    $query_hogar = "SELECT * FROM alumno_hogar WHERE codigo_nie = '$codigo_nie' and codigo_alumno = $codigo_alumno LIMIT 1";
                        ...
                        if($consulta_hogar -> rowCount() != 0){                    
                            ...
                            print "<br>";
                            // query update alumno
                                print $query_alumno_update = "UPDATE alumno SET direccion_alumno = '$direccion', telefono_alumno = '$numero_contacto' WHERE id_alumno = $codigo_alumno";
                            // ejecutar query update alumno.
                                $consulta_hogar_update = $dblink -> query($query_alumno_update);
