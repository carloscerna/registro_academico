...
// variables a utilizar en el encabezado de la tabla para las notas.
      ...
      $salto_competencias_ciudadanas = 7;
...
function Footer()
{
    ...
    //Crear una l�nea de la segunda firma.
    $this->Line(155,200,220,200);
    //Crear una l�nea
    $this->Line(5,207,245,207);
    //Nombre Docente
    $this->SetX(15);
    ...
    //Nombre Director
    $this->RotatedText(160,203,cambiar_de_del($_SESSION['nombre_director']),0,1,'C');
    $this->RotatedText(170,206,'Director(a)',0,1,'C');
   ...
   if($print_codigo_bachillerato >= '01' and  $print_codigo_bachillerato <= '05')
    {
        ...
        $this->RoundedRect(5, 40, 90, 75, 0.5, '1234','');	// para el nombre de la asignatura y las filas
        $this->RoundedRect(95, 40, 80, 10, 0.5, '1234','');	// para los periodos o trimestres
        $this->RoundedRect(95, 45, 85, 5, 0.5, '1234','');	// para las divisiones del los trimestres
        ...
        $this->RoundedRect(175, 40, 45, 75, 0.5, '');		// para el resultado final y las filas
      ...
      $this->RoundedRect(5, 40, 245, 10, 0.5, '');	// primer cuadro.
      $this->RoundedRect(5, 40, 100, 10, 0.5, '');	// para el nombre de la asignatura
      $this->RoundedRect(105, 40, 100, 10, 0.5, '');	// para los periodos o trimestres
	    $this->RoundedRect(205, 45, 45, 5, 0.5, '');	// para las divisiones del los trimestres
      //$this->RoundedRect(75, 45, 140, 5, 0.5, '');	// para las divisiones del los trimestres
      $ancho = 12.5;
		    for($j=0;$j<=7;$j++){
            $this->RoundedRect(105, 45, $ancho, 5, 0.5, '');
      	    $ancho = $ancho + 12.5;			// crear 8 columnas. de los periodos
    	    }
        //$this->RoundedRect(175, 40, 85, 10, 0.5, '');		// para el resultado final
      	//$this->RoundedRect(175, 45, 85, 5, 0.5, '');	// para las divisiones del los trimestres
      	$ancho = 11.25;
		    for($j=0;$j<=3;$j++){
                    $this->RoundedRect(205, 45, $ancho, 5, 0.5, '');
		    $ancho = $ancho + 11.25;}			// crear 4 columnas.
    // Colocar nombres en los cuadros.
      $this->SetFont('Times','B',12);
      $this->RotatedText(38,45,'A S I G N A T U R A S',0);
      $this->SetFont('Times','B',10);
    // Colocar trimestre
      $this->RotatedText(140,44,'P E R I O D O S',0);
      $this->RotatedText(110,49,'1',0);
    	$this->RotatedText(135,49,'2',0);
    	$this->RotatedText(160,49,'3',0);
    	$this->RotatedText(185,49,'4',0);
  
      $this->RotatedText(119,49,'Resul.',0);
    	$this->RotatedText(144,49,'Resul.',0);
    	$this->RotatedText(169,49,'Resul.',0);
    	$this->RotatedText(194,49,'Resul.',0);		
    // Colocar nombre de resultados finales.
      $this->RotatedText(210,44,'RESULTADO FINAL',0);
	    $this->RotatedText(208,49,'T.P.',0);
	    $this->RotatedText(218,49,'P.4.P.',0);
	    $this->RotatedText(228,49,'REPO.',0);
	    $this->RotatedText(240,49,'N.F.',0);		
    } 
...
//************************************************************************************************************************
// Creando el Informe.
    //$pdf=new PDF('L','mm',array(229,162));	// Formato Sobre C5.
...
//  mostrar los valores de la consulta para listado de las notas o solo una.
  ...
  if ($print_codigo_bachillerato >= '01' and $print_codigo_bachillerato <= '05'){
    ...
    $w=array(180,10,80,100,13,12,11); //determina el ancho de las columnas
    $w2=array(5,12); //determina el ancho de las columnas
  }
    ...
    $fill = false; $i=1;  $suma = 0; $aprobado_reprobado = array();
		...
		// imprimir la foto en la boleta
            ...
            // >Impresi�n de la primera asignatura. primer ciclo, segundo y tercero.
            if ($print_codigo_bachillerato >= '01' and $print_codigo_bachillerato <= '05'){
...

///////////////////// CONDICIONES PARA EDUCACI�N MEDIA.
        if ($print_codigo_bachillerato >= '06' and $print_codigo_bachillerato <= '09')
            {
                $pdf->Cell($w[3],$w2[0],$nombre_asignatura,0,0,'L',$fill);
		...
		if($fila_2m !=0)
		{
		   ...
		   while ($row2m=pg_fetch_assoc($result_2m)) // bucle para la recorrer las asignaturas.
		    ...
		    $pdf->Cell(245,0,'','T');
                    $pdf->SetFont('','B',8);
                    $pdf->Ln();
                    $pdf->Cell(140,$w2[0],('Nota. Para Aprobar cada asignatura por Per�odo >= 6.'),0,0,'L');
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            if ($print_codigo_bachillerato >= '03'){

             // IMPRIMIR ASPECTOS DE LA CONDUCTA - EDUCACI�N B�SICA.
             if ($i == $salto_competencias_ciudadanas){
                 //Colores, ancho de l�nea y fuente en negrita
                $pdf->SetFillColor(255,255,255);
                $pdf->SetTextColor(0);
                $pdf->SetFont('Times','B',12);
                $pdf->Cell(245,$w2[0],' C O M P E T E N C I A S    C I U D A D A N A S ',1,0,'C',true);
                $pdf->Ln();
                    //Restauraci�n de colores y fuentes
             ...
             if ($i == $total_asignaturas){
                    ...
                    $pdf->Cell(160,$w2[0],'S� alguna ASIGNATURA aparece en BLANCO consulte con el DOCENTE que la imparte.',0,0,'L');
                    $pdf->Cell(40,$w2[0],'A = Aprobado; R = Reprobado',0,1,'L');
                 //     $pdf->AddPage();
		    if($print_uno == 'no') {
			 // $pdf->AddPage();
