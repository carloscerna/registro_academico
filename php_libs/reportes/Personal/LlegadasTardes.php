...
function Header(){
    ...
    $this->Cell(20);
    //Título
    $this->Cell(0,4,mb_convert_encoding('DEPARTAMENTAL DE EDUCACIÓN DE SANTA ANA',"ISO-8859-1","UTF-8"),0,1,'C');
    $this->Cell(0,4,'REPORTE DE LLEGADAS TARDIAS E INASISTENCIAS INJUSTIFICADAS',0,1,'C');
    $this->Cell(0,4,'CENTRO EDUCATIVO: '.mb_convert_encoding($_SESSION['institucion'],"ISO-8859-1","UTF-8"),0,1,'C');
    $this->Cell(0,4,'INFORME CORRESPONDIENTE AL MES DE: '.$nombre_mes,0,0,'C');
    $this->Cell(0,4,mb_convert_encoding('CÓDIGO: ',"ISO-8859-1","UTF-8").$_SESSION['codigo'],0,0,'R');
    $this->SetXY(0,0);
...
// imprimir los resultados.
		...
		$tramite_minutos = segundosToCadenaM($total_minutos, $calculo_horas);
			$pdf->Cell(200,5.8,'TOTAL',0,0,'R',$fill);
			//$pdf->Cell($w[8],5.8,'',1,0,'C',$fill);
			if($dia_sum == 0){$pdf->Cell($w[8],5.8,'',1,0,'C',$fill);}else{$pdf->Cell($w[8],5.8,$tramite_dia,1,0,'C',$fill);} 	// minutos
			if($horas_sum == 0){$pdf->Cell($w[8],5.8,'',1,0,'C',$fill);}else{$pdf->Cell($w[8],5.8,$tramite_hora,1,0,'C',$fill);} 	// minutos
			if($minutos_sum == 0){$pdf->Cell($w[10],5.8,'',1,0,'C',$fill);}else{$pdf->Cell($w[10],5.8,$tramite_minutos,1,0,'C',$fill);} 	// minutos
		$pdf->Ln(); $pdf->Ln();
// imprimir la fecha.
		$pdf->Cell(200,5.8,'FECHA DE ENTREGA',0,0,'R',$fill);
		$pdf->Cell(30,5.8,cambiaf_a_normal($date),1,0,'C',$fill); 	// minutos
