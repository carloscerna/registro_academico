...
// EJECUTAR CONDICIONES PARA EL NOMBRE DEL NIVEL Y EL N�MERO DE ASIGNATURAS.
	$total_asignaturas = 0;	
	    ...
	    {
		$nivel_educacion = "Educaci�n B�sica";
	    }else{
		// Validar Bachillerato.
		if($print_codigo_bachillerato == '06'){
		    $nivel_educacion = "Educaci�n Media - General";
		}
		
		if($print_codigo_bachillerato == '07'){
		    $nivel_educacion = "Educaci�n Media - T�cnico";
		}
		
		if($print_codigo_bachillerato == '08' or $print_codigo_bachillerato == '09'){
		    $nivel_educacion = "Educaci�n Media - Contadur�a";
		}
		// Validar grado de educaci�n Media.
		if($print_codigo_grado == '10'){
		    $print_grado_media = "Primer a�o";
		}
		if($print_codigo_grado == '11'){
		    $print_grado_media = "Segundo a�o";
		}
		if($print_codigo_grado == '12'){
		    $print_grado_media = "Tercer a�o";
		}
	    }
// Colocar la leyenda del Per�odo.
...
function Footer()
{
    ...
    //N�mero de p�gina
    $this->SetY(-10);
    $this->Cell(0,6,'N� '.$this->PageNo().'/{nb}',0,0,'C');
...
// cuarta PARTE DEL RECTANGULO. asignatura
    ...
    $this->Rect(91,35,84,20);
    $this->Rect(91,35,84,5);
    $this->SetXY(120,35);
...
// OCTAVA PARTE DEL RECTANGULO. Total de Puntos.
    $this->Rect(175,35,8,20);
    $this->SetFont('Arial','',9); // I : Italica; U: Normal;
    $this->RotatedText(180,54,'T.Repo',90);
}
//********************************************************************************************************************************
}
//************************************************************************************************************************
...
// Definimos el tipo de fuente, estilo y tama�o.
        $w2=array(5); //determina el ancho de las columnas
            $pdf->SetFont('Arial','',10); // I : Italica; U: Normal;
             ...
             $pdf->Cell(50,$w2[0],('Secci�n: ').$print_seccion,0,0,'L');
             $pdf->Cell(30,$w2[0],('A�o Lectivo: ').$print_ann_lectivo,0,0,'L');
            $pdf->ln();
...
//  mostrar los valores de la consulta
    ...
    //Cabecera
    $w=array(5,80,7,8,14); //determina el ancho de las columnas
    $w2=array(8,12,70,50,8,4); //determina el ancho de las columnas
        ...
        while($row = $result -> fetch(PDO::FETCH_BOTH))
            ...
            // >Impresi�n de la primera asignatura. primer ciclo, segundo y tercero.
                if ($i == 1){
                    ...
                    if($row[$nota_p] < 5){
			$lblaprobado = cambiar_aprobado_reprobado_b($row[$nota_p]);
			
                        $pdf->SetFont('Arial','B',10); // I : Italica; U: Normal;
                        $pdf->Cell($w[2],$w2[0],$lblaprobado,0,0,'C',$fill);
                        $pdf->SetFont('Arial','',10); // I : Italica; U: Normal;
                    }else{
			$lblreprobado = cambiar_aprobado_reprobado_b(($row[$nota_p]));
			
                        $pdf->SetFont('Arial','',10); // I : Italica; U: Normal;
                        $pdf->Cell($w[2],$w2[0],$lblreprobado,0,0,'C',$fill);
                        $pdf->SetFont('Arial','',10); // I : Italica; U: Normal;                        
                    }
			...
			$lblreprobado = cambiar_aprobado_reprobado_b(($row[$nota_p]));
		     ...
		     if($i == 4){
			   ...
			   $aprobada_sociales[] = $taps;
			      ...
			      if ($row['genero'] == 'f'){$tapsf++;}
                    ...
                    //acumular el valor - educaci�n art�stica o ingles.
		     if($i == 5){
                        if($row[$nota_p] < 5){
		     ...
		     if($i == 6){
			   ...
			   // 
			   $trpf++;
			   $reprobada_fisica[] = $trpf;}
			else{
			   $tapf++;
			   $aprobada_fisica[] = $tapf;
...
// SEPTIMA PARTE DEL RECTANGULO.
    ...
    {    
	    ...
	    $pdf->Cell($w[2],$w2[0],$tapff,0,1,'C',$fill);
	    ////////////////////////////////////////////////////////////////
	    $pdf->Cell($w[0],$w2[0],'',0,0,'C',$fill);
	    ...
	    $pdf->Cell($w[2],$w2[0],$treff,0,1,'C',$fill);
	    ///////////////////////////////////////////////////////////////
	    $pdf->Cell($w[0],$w2[0],'',0,0,'C',$fill);
	    ...
	    $pdf->Cell($w[2],$w2[0],$tapff+$treff,0,1,'C',$fill);
	    //////////////////////////////////////////////////////////////////	    
	    $pdf->Cell($w[0],$w2[0],'',0,0,'C',$fill);
	    ...
	    $pdf->Cell($w[4],$w2[0],count($aprobada_fisica) + count($reprobada_fisica),0,1,'C',$fill);
