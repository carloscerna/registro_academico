...
//  imprimir datos del bachillerato.
     while($row = $result_nombre_asignatura -> fetch(PDO::FETCH_BOTH))
        {
            $data[$j] = substr(utf8_decode(trim($row['nombre_asignatura'])),0,20);
                $j++;
        }
...
function Header()
{
    ...
    //$this->Cell(20);
    //Título
    $this->Cell(260,7,utf8_decode($_SESSION['institucion']),0,1,'C');
    $this->Cell(260,7,'INFORME DE NOTAS POR TRIMESTRE TODAS LAS ASIGNATURAS',0,1,'C');
...
// cuarta PARTE DEL RECTANGULO. asignatura
    ...
    $this->Rect(91,35,180,5);
    $this->SetXY(160,35);
    $this->Cell(60,5,'ASIGNATURA',0,2,'C');
// QUINTA PARTE DEL RECTANGULO. NOMBRE DE LAS ASIGNATURAS
    ...
    for($i=0;$i<=5;$i++)
    {
        ...
        if($i == 0){$relleno = true; $this->SetFillColor(255,255,255);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 1){$relleno = true; $this->SetFillColor(255,255,255);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 2){$relleno = true; $this->SetFillColor(255,255,255);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 3){$relleno = true; $this->SetFillColor(255,255,255);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 4){$relleno = true; $this->SetFillColor(255,255,255);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 5){$relleno = true; $this->SetFillColor(255,255,255);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
		
        //cambiar el color del relleno.
        /*if($i == 0){$relleno = true; $this->SetFillColor(255,165,0);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 1){$relleno = true; $this->SetFillColor(0,250,114);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 2){$relleno = true; $this->SetFillColor(50,205,50);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 3){$relleno = true; $this->SetFillColor(72,118,255);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 4){$relleno = true; $this->SetFillColor(238,238,0);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
        if($i == 5){$relleno = true; $this->SetFillColor(202,255,112);$this->Cell(30,5,$data[$i],0,2,'L',$relleno);}else{$this->SetFillColor(255,255,255);$relleno = false;}
		*/
        $mover_x = $mover_x + 30;
...
// SEXTA PARTE DEL RECTANGULO. NOMBRE DE LOS TRIMESTRES
    ...
    for($i=0;$i<=5;$i++)
    {
        ...
        $this->Rect($x1+$mover_x,$y1,$x2,5);
        $this->SetXY($x1+$mover_x,$y1);
        $this->Cell(30,5,'Trimestre',0,2,'L');
...
// OCTAVA PARTE DEL RECTANGULO. Total de Puntos.
    $this->Rect(271,35,8,20);
    $this->SetFont('Arial','',9); // I : Italica; U: Normal;
    $this->RotatedText(275,54,'T.Puntos',90);
}
//********************************************************************************************************************************
}
//************************************************************************************************************************
        ...
        while($row = $result -> fetch(PDO::FETCH_BOTH))
            ...
            // >Impresión de la primera asignatura. primer ciclo, segundo y tercero.
                if ($i == 1){
                    ...
                    $pdf->Cell($w[2],$w2[0],number_format(trim($row['total_puntos_basica']),1),0,0,'C',$fill);
                    ...
                    $total_puntos = $total_puntos + $row['total_puntos_basica'];
                }
