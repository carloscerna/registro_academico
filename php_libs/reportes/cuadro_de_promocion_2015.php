...
// tercera PARTE DEL RECTANGULO.   nombre del alumno
    ...
    $this->SetFont('Arial','',16); // I : Italica; U: Normal;
    $this->SetXY(18,25);
...
// cuarta PARTE DEL RECTANGULO. educacion moral y civica
    $this->Rect(187,5,50,7);
    $this->SetXY(182,5);
    $this->SetFont('Arial','',9); // I : Italica; U: Normal;
    $this->Cell(60,4,utf8_decode('EDUCACIÓN MORAL Y CÍVICA'),0,2,'C');
...
// cuarta PARTE DEL RECTANGULO. calificacion
    ...
    $this->SetFont('Arial','',7); // I : Italica; U: Normal;
      ...
      $this->RotatedText(140,52,utf8_decode('CALIFICACIÓN'),0);
// cuarta PARTE DEL RECTANGULO. aspectos de la conducta
    $espacio = 0;
    for($i=1;$i<=5;$i++){
      $this->Rect(187+$espacio,12,10,43);
      $espacio = $espacio + 10;}
}
}
//Pie de página
...
// Imprimir el primer encabezado EL ESCUDO DE EL SALVADOR... Y TEXTO. 3 LINEAS
    $img = $_SERVER['DOCUMENT_ROOT'].'/registro_web/img/escudo.jpg';
...
// cuarta PARTE DEL RECTANGULO. educacion moral y civica
    $pdf->Rect(187,45,50,7);
    $pdf->SetXY(182,45);
    $pdf->SetFont('Arial','',9); // I : Italica; U: Normal;
    $pdf->Cell(60,4,utf8_decode('EDUCACIÓN MORAL Y CÍVICA'),0,2,'C');
...
// cuarta PARTE DEL RECTANGULO. calificacion
    ...
    /*for($i=1;$i<=6;$i++){
      ...
      $pdf->RotatedText(140,92,utf8_decode('CALIFICACIÓN'),0);

// cuarta PARTE DEL RECTANGULO. aspectos de la conducta
    $espacio = 0;
    for($i=1;$i<=5;$i++){
      $pdf->Rect(187+$espacio,52,10,43);
      $espacio = $espacio + 10;}      

// cuarta PARTE DEL RECTANGULO. Escala de Calificación.  y texto.
    $pdf->Rect(250,50,85,40);
    $pdf->SetFillColor(206,206,206);
    $pdf->Rect(250,50,85,15,"F");
    $pdf->SetFont('Arial','',10); // I : Italica; U: Normal;
    
    $pdf->SetXY(250,50);
    $pdf->Cell(85,4.5,utf8_decode('ESCALA DE CALIFICACIONES PARA'),0,2,'C');
    $pdf->Cell(85,4.5,utf8_decode('EDUCACIÓN MORAL Y CÍVICA'),0,2,'C');
    $pdf->Cell(85,4.5,'Aspectos de la Conducta',0,2,'C');

    $pdf->SetXY(250,68);
    $pdf->SetFont('Arial','b',13);
    $pdf->Cell(85,6,'E: Excelente',0,2,'C');
    $pdf->Cell(85,6,'MB: Muy Bueno',0,2,'C');
    $pdf->Cell(85,6,'B: Bueno',0,2,'C');

// cuarta PARTE DEL RECTANGULO. ESTADISTICA Y TEXTO.
    $pdf->Rect(245,150,95,30);
      $pdf->Rect(245,150,20,40);
    $pdf->Rect(245,150,95,10);
      $pdf->Rect(265,150,15,40);
    $pdf->Rect(245,160,95,10);
      $pdf->Rect(280,150,15,40);
    $pdf->Rect(245,170,95,10);
      $pdf->Rect(295,150,15,40);
    $pdf->Rect(245,180,95,10);
      $pdf->Rect(310,150,15,40);
    
    $pdf->SetXY(250,145);
    $pdf->Cell(85,4.5,utf8_decode('ESTADÍSTICA'),0,2,'C');
    $pdf->SetXY(245,155);
    $pdf->Cell(20,4.5,'SEXO',0,0,'C');
    $pdf->SetFont('Arial','',7);
    $pdf->SetXY(265,151);
    ...
    $pdf->SetXY(248,165);
    $pdf->Cell(15,5,'Masculino',0,0,'R');
    $pdf->SetFont('Arial','',12);
    $pdf->SetXY(295,165);
    $pdf->Cell(15,5,$total_alumnos_masculino,0,0,'C');
    $pdf->SetFont('Arial','',10);
    $pdf->SetXY(248,175);
    $pdf->Cell(15,5,'Femenino',0,0,'R');
    $pdf->SetXY(295,175);
    $pdf->SetFont('Arial','',12);
    ...
    $pdf->SetXY(248,195);
    $pdf->Cell(25,5,'PROMOVIDOS:     '.utf8_decode(strtolower(num2letras($total_promovidos_f+$total_promovidos_m))),0,0,'L');
    $pdf->SetXY(248,205);
    $pdf->Cell(25,5,'RETENIDOS:     '.strtolower(num2letras($total_retenidos_f+$total_retenidos_m)),0,0,'L');
