...
// tercera PARTE DEL RECTANGULO.   nombre del alumno
    ...
    $this->MultiCell(90,8,convertirtexto('Nombre de los Alumnos(as) en orden alfabético de apellidos'),0,2,'C');
// cuarta PARTE DEL RECTANGULO. nie
    //$this->Rect(107,45,20,50);
    //$this->SetXY(110,65);
    //$this->Cell(10,8,'NIE',0,2,'C');
// cuarta PARTE DEL RECTANGULO. asignatura
    $this->SetFont('Arial','',13); // I : Italica; U: Normal;
    $this->Rect(127,5,60,7);
    $this->SetXY(132,5);
    $this->Cell(50,8,'ASIGNATURA',0,2,'C');
// cuarta PARTE DEL RECTANGULO. educacion moral y civica
    $this->Rect(187,5,40,7);
    $this->SetXY(187,5);
    $this->SetFont('Arial','',7); // I : Italica; U: Normal;
    $this->Cell(40,8,convertirtexto('COMPETENCIAS CIUDADANAS'),0,2,'C');
...
// cuarta PARTE DEL RECTANGULO. asignaturas nombres
    ...
    for($ix=0;$ix<=$total_asignaturas-1;$ix++){
        if($ix >= 0 && $ix <= 5){
          ...
          $this->RotatedTextMultiCell(127+$espacio,44,$nombre_asignatura[$ix],90);
        }
        else{
          $this->RotatedTextMultiCellAspectos(127+$espacio,54,$nombre_asignatura[$ix],90);
        }
          $espacio = $espacio + 10;
      }
// cuarta PARTE DEL RECTANGULO. calificacion
    $espacio = 0;
      ...
      $this->Rect(127,45,60,10);
      $this->RotatedText(143,52,convertirtexto('CALIFICACIÓN'),0);
...
// Imprimir el primer encabezado REGISTRO DE EVALUACION....
    ...
    $pdf->Cell(235,14,convertirtexto('REGISTRO DE EVALUACIÓN DEL RENDIMIENTO ESCOLAR DE '.substr($codigo_grado,1,1).'.° DE EDUCACIÓN BÁSICA'),0,0,'L');
...
// Imprimir el primer encabezado EL ESCUDO DE EL SALVADOR... Y TEXTO. 3 LINEAS
    ...
    $pdf->Cell(60,4,convertirtexto('Dirección Nacional de Educación Básica'),0,2,'C');
// PRIMERA PARTE DEL RECTANGULO.
    $pdf->Rect(10,45,217,50);
// segunda PARTE DEL RECTANGULO. numero de orden
    $pdf->Rect(10,45,7,50);
...
// tercera PARTE DEL RECTANGULO.   nombre del alumno
    ...
    $pdf->MultiCell(90,8,convertirtexto('Nombre de los Alumnos(as) en orden alfabético de apellidos'),0,2,'C');
// cuarta PARTE DEL RECTANGULO. nie
...
// cuarta PARTE DEL RECTANGULO. asignatura
    ...
    $pdf->Rect(127,45,60,7);
    $pdf->SetXY(132,45);
    $pdf->Cell(50,8,'ASIGNATURA',0,2,'C');
// cuarta PARTE DEL RECTANGULO. educacion moral y civica
    $pdf->Rect(187,45,40,7);
    $pdf->SetXY(187,45);
    $pdf->SetFont('Arial','',7); // I : Italica; U: Normal;
    $pdf->Cell(40,8,convertirtexto('COMPETENCIAS CIUDADANAS'),0,2,'C');
...
// cuarta PARTE DEL RECTANGULO. calificacion
    $espacio = 0;
      ...
      $pdf->Rect(127,85,60,10);
      $pdf->RotatedText(145,92,convertirtexto('CALIFICACIÓN'),0);
// cuarta PARTE DEL RECTANGULO. aspectos de la conducta
    $espacio = 0;
    for($i=1;$i<=3;$i++){
...
// cuarta PARTE DEL RECTANGULO. ESTADISTICA Y TEXTO.
    ...
    if(($total_promovidos_f + $total_promovidos_m) == 0){
      $pdf->Cell(75,5,strtolower("cero"),0,0,'C');
    }else{
      $pdf->Cell(75,5,strtolower(convertirTexto(num2letras($total_promovidos_f+$total_promovidos_m))),0,0,'C');
    }
    ...
    if($total_retenidos_m_f == 0){
      ...
      $pdf->Cell(75,5,strtolower(convertirTexto(num2letras($total_retenidos_f+$total_retenidos_m))),0,0,'C');  
    }
///////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
//  INICIO PARA MOSTRAR LOS DATOS DE LA TABLA.
    //Datos de la institucion.
            ...
            $pdf->Cell(85,5.5,convertirtexto(substr($codigo_grado,1,1).'.° GRADO.      SECCIÓN: '.$nombre_seccion.'    CÓDIGO DE INFRAESTRUCTURA: '.$_SESSION['codigo']),0,2,'L');
            ...
            switch($i)
            {
              case 1:
                ...
                $pdf->SetX(10);
                  ...
                  $pdf->Cell(90,$h[0],cambiar_de_del(trim($row['apellido_alumno'])),1,0,'l',$fill);  // nombre del alumno.
              ...
              case 6:
                $nota_final_ = verificar_nota($nota_final, $nota_r_1, $nota_r_2);
                $total_puntos_06_array[] = $nota_final_;
                if($nota_final_ < 5 ){
                  $pdf->SetTextColor(255,0,0);
                    $pdf->Cell(10,$h[0],$nota_final_,1,0,'C',$fill); 
                  $pdf->SetTextColor(0,0,0);
              ...
              case ($i>=7 and $i<=10):
                ...
                $pdf->Cell(10,$h[0],$concepto_asignatura,1,0,'C',$fill);
                // salto de linea.
                  if($i == 10){
                    $pdf->Ln();
                  }
		   ...
		   // Para el fondo de la fila.
		      $fill=!$fill;		    
                    $pdf->SetX(10);
                      ...
                      for($j=0;$j<=8;$j++){$pdf->Cell(10,$h[0],'',1,0,'C');}
                      $pdf->Ln();
           ...
           // Ultimas lineas....
              $pdf->SetX(10);
                $pdf->Cell(117,$h[0],'TOTAL DE PUNTOS',1,0,'R');  // TOTAL DE PUNTOS
                  ...
                  for($j=0;$j<=3;$j++){$pdf->Cell(10,$h[0],'',1,0,'C');}
                    $pdf->Ln();
                    $pdf->SetX(10);
